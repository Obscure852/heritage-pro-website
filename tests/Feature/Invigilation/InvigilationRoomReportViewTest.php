...
class InvigilationRoomReportViewTest extends TestCase
{
    ...
    public function test_room_roster_defaults_to_timetable_layout(): void
    {
        ...
        $response = $this->actingAs($user)->get(route('invigilation.reports.room.index', ['series_id' => $series->id]));

        $response->assertOk()
            ->assertSee('Room Timetable')
            ->assertSee('Room')
            ...
            ->assertSee(route('invigilation.reports.room.index', [
                'series_id' => $series->id,
                'layout' => 'timetable',
                'print' => 1,
            ]));
    ...
    public function test_room_roster_table_layout_renders_grouped_table_and_preserves_layout_selector(): void
    {
        [$term, $gradeSubject] = $this->createAcademicContext();
        $series = $this->createSeries($term);
        $user = $this->createUser();
        ...
        $response = $this->actingAs($user)->get(route('invigilation.reports.room.index', ['series_id' => $series->id, 'layout' => 'table']));
        ...
        $response->assertOk()
            ...
            ->assertDontSee('Room Timetable');
    }

    public function test_room_roster_table_print_view_includes_auto_print_script(): void
    {
        [$term, $gradeSubject] = $this->createAcademicContext();
        $series = $this->createSeries($term);
        $user = $this->createUser();
        ...
        $response = $this->actingAs($user)->get(route('invigilation.reports.room.index', [
            'series_id' => $series->id,
            'layout' => 'table',
            'print' => 1,
        ]));
    ...
    public function test_room_roster_timetable_print_view_includes_landscape_print_css_and_auto_print_script(): void
    {
        [$term, $gradeSubject] = $this->createAcademicContext();
        $series = $this->createSeries($term);
        $user = $this->createUser();
    ...
    private function createUser(): User
    {
        return User::withoutEvents(fn () => User::query()->create([
            ...
            'lastname' => 'Viewer',
            'email' => 'room.report.' . uniqid() . '@example.com',
            'password' => 'secret',
            'status' => 'Current',
            'position' => 'Teacher',
