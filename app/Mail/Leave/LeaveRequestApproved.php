...
/**
 * Mailable for staff notification on leave request approval (NOTF-03).
 *
 * Sent to staff when their leave request has been approved by their manager.
 */
class LeaveRequestApproved extends Mailable implements ShouldQueue {
    ...
    /** @var float Total days approved */
    public float $totalDays;

    /** @var string Approver's name */
    public string $approverName;

    /** @var string|null Approver comments */
    ...
    public function __construct(LeaveRequest $request) {
        ...
        $this->comments = $request->approver_comments;
    ...
    public function envelope(): Envelope {
        return new Envelope(
            subject: "Leave Request Approved - {$this->leaveType} ({$this->startDate} to {$this->endDate})",
        );
    }
    ...
    public function build() {
        return $this->subject("Leave Request Approved - {$this->leaveType} ({$this->startDate} to {$this->endDate})")
                    ->view('emails.leave.approved');
    }
