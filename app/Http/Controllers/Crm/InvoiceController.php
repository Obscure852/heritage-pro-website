...
use App\Http\Requests\Crm\InvoiceUpsertRequest;
...
class InvoiceController extends CrmController
{
    ...
    public function index(Request $request): View
    {
        ...
        $baseQuery = $this->invoiceIndexQuery();
        ...
        return view('crm.products.invoices.index', [
            ...
            'canCreateInvoices' => $this->crmUser()->canIssueCommercialInvoices(),
        ]);
    }

    public function create(Request $request): View
    {
        $this->authorizeInvoiceManagement();

        return view('crm.products.invoices.create', $this->formData(null, $request));
    }

    public function store(InvoiceUpsertRequest $request): RedirectResponse
    {
        $this->authorizeInvoiceManagement();

        $invoice = DB::transaction(function () use ($request) {
            return $this->persistInvoice($request->validated());
        });

        return redirect()
            ->route('crm.products.invoices.show', $invoice)
            ->with('crm_success', 'Invoice created successfully.');
    }

    public function show(CrmInvoice $crmInvoice): View
    {
        $this->authorizeInvoiceAccess($crmInvoice);
        ...
        return view('crm.products.invoices.show', [
            'invoice' => $crmInvoice,
            'invoiceStatuses' => config('heritage_crm.invoice_statuses', []),
            'discountTypes' => config('heritage_crm.commercial_discount_types', []),
            'canEditInvoice' => $this->canEditInvoice($crmInvoice),
            'canShareInvoice' => $this->canShareInvoice($crmInvoice),
            'availableTransitions' => $this->crmUser()->canIssueCommercialInvoices()
                ? $this->availableTransitions($crmInvoice)
                : [],
        ]);
    }

    public function edit(CrmInvoice $crmInvoice): View
    {
        $this->authorizeInvoiceEditing($crmInvoice);

        return view('crm.products.invoices.edit', $this->formData($crmInvoice));
    }

    public function update(InvoiceUpsertRequest $request, CrmInvoice $crmInvoice): RedirectResponse
    {
        $this->authorizeInvoiceEditing($crmInvoice);

        DB::transaction(function () use ($request, $crmInvoice) {
            $this->persistInvoice($request->validated(), $crmInvoice);
        });

        return redirect()
            ->route('crm.products.invoices.edit', $crmInvoice)
            ->with('crm_success', 'Invoice updated successfully.');
    }

    public function transition(Request $request, CrmInvoice $crmInvoice): RedirectResponse
    {
        $this->authorizeInvoiceLifecycle($crmInvoice);
        ...
        $allowedTransitions = $this->availableTransitions($crmInvoice);
        ...
        if ($targetStatus === 'issued') {
            $updates['issued_at'] = $crmInvoice->issued_at ?: now();
        ...
        $crmInvoice->update($updates);

        return redirect()
            ->route('crm.products.invoices.show', $crmInvoice)
            ->with('crm_success', 'Invoice status updated to ' . strtolower($allowedTransitions[$targetStatus]) . '.');
    }

    public function openPdf(CrmInvoice $crmInvoice): BinaryFileResponse
    {
        $this->authorizeInvoiceAccess($crmInvoice);

        return $this->pdfService->openResponse(
            $this->pdfService->ensureInvoiceArtifact($crmInvoice, $this->crmUser())
        );
    }

    public function downloadPdf(CrmInvoice $crmInvoice): BinaryFileResponse
    {
        $this->authorizeInvoiceAccess($crmInvoice);
    ...
    public function shareCreate(CrmInvoice $crmInvoice): View
    {
        $this->authorizeInvoiceAccess($crmInvoice);
        abort_unless($this->canShareInvoice($crmInvoice), 403);

        return view('crm.products.invoices.share', [
            'invoice' => $crmInvoice,
    ...
    public function shareStore(DiscussionThreadStoreRequest $request, CrmInvoice $crmInvoice): RedirectResponse
    {
        $this->authorizeInvoiceAccess($crmInvoice);
        abort_unless($this->canShareInvoice($crmInvoice), 403);

        $thread = $this->shareService->shareInvoice(
            $crmInvoice,
            $this->crmUser(),
            $request->validated()
        );
        ...
        return redirect()
            ...
            ->with('crm_success', 'Invoice shared successfully.');
    }

    private function persistInvoice(array $payload, ?CrmInvoice $existingInvoice = null): CrmInvoice
    {
        ...
        $invoicePayload = [
            'owner_id' => $this->syncedOwnerId($lead, $customer, $existingInvoice?->owner_id),
            'lead_id' => $lead?->id,
            'customer_id' => $customer?->id,
            'contact_id' => $contact?->id,
            'request_id' => $crmRequest?->id,
            'invoice_number' => $existingInvoice?->invoice_number ?? $this->numberingService->nextInvoiceNumber(),
            'status' => $existingInvoice?->status ?? 'draft',
            'invoice_date' => $payload['invoice_date'],
        ];

        $this->validationService->validateInvoice($invoicePayload);

        $linePayload = $this->normalizedLinePayload(
            $payload['items'],
            $settings->allow_line_discounts,
            (float) $settings->default_tax_rate,
            $existingInvoice?->items()->pluck('product_id')->filter()->map(fn ($id) => (int) $id)->values()->all() ?? []
        );
        ...
        $invoiceAttributes = [
            'owner_id' => $invoicePayload['owner_id'],
            'lead_id' => $invoicePayload['lead_id'],
            'customer_id' => $invoicePayload['customer_id'],
            'contact_id' => $invoicePayload['contact_id'],
            'request_id' => $invoicePayload['request_id'],
        ...
        $invoice = $existingInvoice ?? new CrmInvoice();
        ...
        return $invoice->fresh(['owner', 'lead', 'customer', 'contact', 'request', 'items.product']);
    }
            ...
            ->map(function (array $item, int $index) use ($allowLineDiscounts, $defaultTaxRate, $allowedInactiveProductIds) {
                ...
                if ($product !== null && ! $product->active && ! in_array((int) $product->id, $allowedInactiveProductIds, true)) {
                    throw ValidationException::withMessages([
                        'items.' . $index . '.product_id' => 'Inactive products cannot be used on new invoices.',
                    ]);
                }
                ...
                if ($itemName === '') {
                    throw ValidationException::withMessages([
                        'items' => 'Every invoice line must include an item name or selected product.',
                    ]);
                }
    ...
    private function formData(?CrmInvoice $crmInvoice = null, ?Request $request = null): array
    {
        $crmInvoice?->loadMissing('items.product');
        ...
        if ($crmInvoice !== null) {
            $historicalInactiveProducts = $crmInvoice->items
                ->pluck('product')
    ...
    private function invoiceIndexQuery(): Builder
    {
    ...
    private function authorizeInvoiceManagement(): void
    {
        abort_unless($this->crmUser()->canIssueCommercialInvoices(), 403);
    }

    private function authorizeInvoiceAccess(CrmInvoice $crmInvoice): void
    {
        abort_unless($this->crmUser()->canAccessCommercialDocumentRecord($crmInvoice->owner_id), 403);
    }

    private function authorizeInvoiceEditing(CrmInvoice $crmInvoice): void
    {
        $this->authorizeInvoiceAccess($crmInvoice);
        $this->authorizeInvoiceManagement();
        abort_unless($this->canEditInvoice($crmInvoice), 403);
    }

    private function authorizeInvoiceLifecycle(CrmInvoice $crmInvoice): void
    {
        $this->authorizeInvoiceAccess($crmInvoice);
        $this->authorizeInvoiceManagement();
    }

    private function canEditInvoice(CrmInvoice $crmInvoice): bool
    {
        return $this->crmUser()->canIssueCommercialInvoices()
            && in_array($crmInvoice->status, $this->editableStatuses(), true);
    }

    private function canShareInvoice(CrmInvoice $crmInvoice): bool
    {
        return $this->crmUser()->canIssueCommercialInvoices()
            && in_array($crmInvoice->status, ['issued', 'sent'], true);
    }
    ...
    private function availableTransitions(CrmInvoice $crmInvoice): array
    {
        return match ($crmInvoice->status) {
            'draft' => [
                'issued' => 'Issued',
                'cancelled' => 'Cancelled',
                'void' => 'Void',
