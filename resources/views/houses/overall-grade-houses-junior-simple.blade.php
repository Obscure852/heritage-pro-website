...
@section('title')
    Grade Performance Analysis (Simple)
@endsection
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="report-card">
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="mb-3">{{ $grade->name ?? '' }} Grade House Performance Analysis -
                    ...
                    <div class="row no-print mt-4">
                        ...
                        <div class="col-md-6">
                            <h5 class="text-center">Subject Grade Distribution</h5>
                            <select id="subjectSelect" class="form-select form-select-sm mb-3">
                                @foreach ($allSubjects as $subject)
                                    <option value="{{ $subject }}">{{ $subject }}</option>
                                @endforeach
                            </select>
                            <div id="subjectGradeChart" style="width: 100%; height: 350px;"></div>
                        </div>
                    </div>
...
<script>
    ...
    // 4. Subject Grade Distribution Chart (Bar)
    ...
    function updateSubjectChart(subjectName) {
        ...
        var gradeColors = ['#91cc75', '#5470c6', '#fac858', '#fc8452', '#ee6666', '#909399'];
        var series = subjectGrades.map(function(grade, index) {
            return {
                name: grade,
                type: 'bar',
                stack: 'total',
                data: houseNames.map(function(house) {
                    return housePerformance[house]['subjectGradeCounts'][subjectName][grade];
                }),
                itemStyle: { color: gradeColors[index] }
            };
        });
        ...
        subjectChart.setOption({
            title: { text: 'Grade Distribution for ' + subjectName, left: 'center' },
            tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
            legend: { top: 30, data: subjectGrades },
            grid: { top: 80, containLabel: true },
            xAxis: { type: 'category', data: houseNames },
            yAxis: { type: 'value', name: 'Students', minInterval: 1 },
            series: series
