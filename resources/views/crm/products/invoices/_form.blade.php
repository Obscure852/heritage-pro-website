@php
    ...
    if (! $defaultAccountType) {
        if (($invoice->lead_id ?? null) || ($defaultSelections['lead_id'] ?? null)) {
    ...
    $selectedLeadId = old('lead_id', $invoice->lead_id ?? $defaultSelections['lead_id']);
    $selectedCustomerId = old('customer_id', $invoice->customer_id ?? $defaultSelections['customer_id']);
    $selectedContactId = old('contact_id', $invoice->contact_id ?? $defaultSelections['contact_id']);
    $selectedRequestId = old('request_id', $invoice->request_id ?? $defaultSelections['request_id']);
...
<form method="POST" action="{{ $action }}" class="crm-form" id="invoice-form">
    @csrf
    @if (! empty($method))
        @method($method)
    @endif
    ...
    <div class="crm-help">
        Invoices can be linked to exactly one lead or customer, one recipient contact, and an optional sales request. Catalog values are copied into the invoice at save time so later product edits do not change historical billing records.
    </div>
    ...
    <section class="crm-card">
        <div class="crm-card-title">
            <div>
                <p class="crm-kicker">Invoice context</p>
        ...
        <div class="crm-field-grid">
            ...
            <div class="crm-field">
                ...
                <input id="invoice_date" name="invoice_date" type="date" value="{{ old('invoice_date', isset($invoice) ? $invoice->invoice_date?->format('Y-m-d') : now()->format('Y-m-d')) }}" required>
            </div>
            <div class="crm-field full">
                <label for="subject">Subject</label>
                <input id="subject" name="subject" value="{{ old('subject', $invoice->subject ?? '') }}" placeholder="Billing summary, renewal reference, or invoice note">
            </div>
            <div class="crm-field">
                ...
                @if ($settings->allow_document_discounts)
                    <select id="document_discount_type" name="document_discount_type" data-document-discount-type>
                        @foreach ($discountTypes as $value => $label)
                            <option value="{{ $value }}" @selected(old('document_discount_type', $invoice->document_discount_type ?? 'none') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                @else
            ...
            <div class="crm-field">
                ...
                @if ($settings->allow_document_discounts)
                    <input id="document_discount_value" name="document_discount_value" type="number" step="0.01" min="0" value="{{ old('document_discount_value', isset($invoice) ? number_format((float) $invoice->document_discount_value, 2, '.', '') : '0.00') }}" data-document-discount-value>
                @else
            ...
            <div class="crm-field full">
                ...
                <textarea id="notes" name="notes" placeholder="Add internal billing notes, renewal context, or finance-only commentary">{{ old('notes', $invoice->notes ?? '') }}</textarea>
            </div>
            <div class="crm-field full">
                <label for="terms">Invoice terms</label>
                <textarea id="terms" name="terms" placeholder="Add payment terms, implementation notes, or billing assumptions">{{ old('terms', $invoice->terms ?? '') }}</textarea>
            </div>
        </div>
    </section>
    ...
    <section class="crm-card">
        <div class="crm-card-title">
            <div>
                ...
                <h2>Invoice composition</h2>
                <p>Mix catalog products with manual billing lines. Values below are snapshotted into the invoice on save.</p>
            </div>
            <div class="crm-page-tools">
                <button type="button" class="btn btn-light crm-btn-light" id="add-invoice-line">
                    <i class="bx bx-plus"></i> Add line
                </button>
            </div>
        </div>

        <div class="crm-list" id="invoice-line-list">
            @foreach ($formLineItems as $index => $item)
                <div class="crm-card" data-invoice-line data-line-index="{{ $index }}" style="padding: 18px;">
                    ...
                    <div class="crm-field-grid">
                        ...
                        <div class="crm-field full">
                            ...
                            <textarea name="items[{{ $index }}][item_description]" data-line-description placeholder="Optional scope or billing detail">{{ $item['item_description'] ?? '' }}</textarea>
                        </div>
                    </div>
        ...
        <template id="invoice-line-template">
            <div class="crm-card" data-invoice-line data-line-index="__INDEX__" style="padding: 18px;">
                <div class="crm-inline" style="justify-content: space-between; margin-bottom: 16px;">
                ...
                <div class="crm-field-grid">
                    ...
                    <div class="crm-field full">
                        <label>Description</label>
                        <textarea name="items[__INDEX__][item_description]" data-line-description placeholder="Optional scope or billing detail"></textarea>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var lineList = document.getElementById('invoice-line-list');
        ...
        function lineNodes() {
            return Array.from(lineList.querySelectorAll('[data-invoice-line]'));
        }
