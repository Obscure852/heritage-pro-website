    ...
    @endif
            ...
            <div class="settings-container">
                <div class="settings-header">
                    <h3><i class="bx bx-edit me-2"></i>Edit Subject</h3>
                    <p>Modify the master subject details</p>
                ...
                <div class="settings-body">
                    <div class="help-text">
                        <p><i class="bx bx-info-circle"></i>Update the subject details below. Changes will apply to all grade allocations using this master subject.</p>
                    </div>

                    <form class="needs-validation" method="post"
                        action="{{ route('subjects.update-master-subject', $subject->id) }}" id="subjectForm" novalidate>
                        @csrf
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="abbrev">Subject Code <span class="required">*</span></label>
                                <input type="text" name="abbrev" id="abbrev" class="form-control"
                                    placeholder="e.g., MATH" value="{{ old('abbrev', $subject->abbrev) }}" required>
                            </div>

                            <div class="form-group">
                                <label for="name">Subject Name <span class="required">*</span></label>
                                <input type="text" name="name" id="name" class="form-control"
                                    placeholder="e.g., Mathematics" value="{{ old('name', $subject->name) }}" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="level" id="level" class="form-select" required>
                                    ...
                                    <option value="Preschool" {{ old('level', $subject->level) == 'Preschool' ? 'selected' : '' }}>Preschool</option>
                                    <option value="Primary" {{ old('level', $subject->level) == 'Primary' ? 'selected' : '' }}>Primary</option>
                                    <option value="Junior" {{ old('level', $subject->level) == 'Junior' ? 'selected' : '' }}>Junior</option>
                                    <option value="Senior" {{ old('level', $subject->level) == 'Senior' ? 'selected' : '' }}>Senior</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="components" id="components" class="form-select" required>
                                    ...
                                    <option value="1" {{ old('components', $subject->components) == '1' ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ old('components', $subject->components) == '0' ? 'selected' : '' }}>No</option>
                                </select>
                            </div>

                            <div class="form-group" style="padding-top: 28px;">
                                <div class="form-check">
                                    <input type="checkbox" name="is_double" id="is_double" class="form-check-input" value="1" {{ old('is_double', $subject->is_double) ? 'checked' : '' }}>
