...
@section('content')
    ...
    <div class="form-container">
        <div class="page-header">
            <h1 class="page-title">Add Qualification for {{ $user->full_name }}</h1>
        </div>

        <div class="help-text">
            <div class="help-title">New Qualification</div>
            <div class="help-content">
                Add educational qualifications and certifications for this staff member. All fields marked with <span class="text-danger">*</span> are required.
            </div>
        </div>

        <form class="needs-validation" method="post" action="{{ route('staff.store-x-qualification') }}" novalidate>
            @csrf
            ...
            <div class="form-group">
                <label class="form-label" for="level">Qualification Level <span class="required">*</span></label>
                <select name="level" id="level" class="form-select @error('level') is-invalid @enderror" required>
                    <option value="">Select Qualification Level</option>
                    <option value="Postgrade" {{ old('level') == 'Postgrade' ? 'selected' : '' }}>Postgraduate</option>
            ...
            <div class="form-group">
                <label class="form-label" for="qualification_id">Qualification <span class="required">*</span></label>
                <select name="qualification_id" id="qualification_id" class="form-select @error('qualification_id') is-invalid @enderror" required>
                    <option value="">Select Qualification</option>
                    @foreach ($qualifications as $qualification)
                        <option value="{{ $qualification->id }}" {{ old('qualification_id') == $qualification->id ? 'selected' : '' }}>
                            {{ $qualification->qualification_code }} | {{ $qualification->qualification }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control @error('completion_date') is-invalid @enderror"
                    name="completion_date" id="completion_date" value="{{ old('completion_date') }}" required>
            </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn btn-primary btn-loading">
                    <span class="btn-text">
                        <i class="fas fa-save me-1"></i> Save Qualification
                    </span>
