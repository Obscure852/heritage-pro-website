...
@section('content')
    @php
        ...
        $timetableUrl = route('invigilation.reports.room.index', array_merge($baseQuery, ['layout' => 'timetable']));
        $tableUrl = route('invigilation.reports.room.index', array_merge($baseQuery, ['layout' => 'table']));
        $csvUrl = $series ? route('invigilation.reports.room.index', ['series_id' => $series->id, 'format' => 'csv']) : '#';
        $printUrl = $series ? route('invigilation.reports.room.index', ['series_id' => $series->id, 'layout' => $activeLayout, 'print' => 1]) : '#';
    ...
    <div class="invigilation-report-container printable">
        <div class="invigilation-header">
            <div class="row align-items-center">
                <div class="col-md-7">
                    <h3 class="mb-1 text-white">Room Roster</h3>
                ...
                <div class="col-md-5">
                    <div class="module-header-actions invigilation-daily-actions">
                        <div class="invigilation-daily-action-cluster" role="group" aria-label="Room roster actions">
                            <div class="btn-group invigilation-layout-toggle" role="group" aria-label="Room roster layout">
                                <a
                                    href="{{ $timetableUrl }}"
        ...
        <div class="invigilation-report-body">
            @include('invigilation.partials.module-nav', ['current' => 'room', 'series' => $series])
            @include('invigilation.partials.report-selector', [
                'action' => route('invigilation.reports.room.index'),
            ...
            @if (!$series)
                @include('invigilation.partials.empty-state', [
                    ...
                    'copy' => 'Create a series first, then return here to view room coverage.',
                ])
            @elseif ($isTimetableLayout)
                @if (!$hasTimetableRows)
                    @include('invigilation.partials.empty-state', [
                        'icon' => 'fas fa-door-open',
                        'title' => 'No room roster data is available yet.',
                        'copy' => 'Add sessions and room allocations to start building the room roster.',
                    ])
                @else
                    <div class="card-shell invigilation-timetable-shell">
                        <div class="card-body p-0">
                            <div class="invigilation-section-header invigilation-timetable-header">
                                <div>
                                    <h5 class="invigilation-section-title">Room Timetable</h5>
                                    <p class="invigilation-section-subtitle">Rooms run down the left and dates run across the top, with session tiles inside each room/day cell ordered by time.</p>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive invigilation-timetable-scroll">
                                <table class="table invigilation-timetable mb-0">
                                    <thead>
                                        <tr>
                                            <th class="invigilation-timetable-row-col">Room</th>
                                        ...
                                        @foreach ($timetable['resource_rows'] as $resourceRow)
                                            <tr>
                                                <th class="invigilation-timetable-row-col">
                                                    ...
                                                    <div class="invigilation-timetable-row-meta">{{ $resourceRow['count'] }} session(s)</div>
                                                </th>
                                                @foreach ($timetable['dates'] as $date)
                                                    @php
                                                            ...
                                                            <div class="invigilation-slot-stack">
                                                                @foreach ($cellRows as $row)
                                                                    ...
                                                                    <article class="invigilation-slot-tile {{ $isCovered ? 'tile-covered' : 'tile-pending' }}">
                                                                        ...
                                                                        <div class="invigilation-slot-meta">{{ $row['group'] ?: 'No group' }} | {{ $row['candidate_count'] }} candidate(s)</div>
                ...
                @endif
            @else
                @forelse ($rows as $venue => $items)
                    <div class="card-shell mb-4">
                        <div class="card-body p-4">
                            <div class="invigilation-section-header">
                                <div>
                                    <h5 class="invigilation-section-title">{{ $venue }}</h5>
                                    <p class="invigilation-section-subtitle">{{ count($items) }} session(s)</p>
                                </div>
                            </div>
