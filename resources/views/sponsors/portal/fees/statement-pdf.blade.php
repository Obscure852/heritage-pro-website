...
<body>
    ...
    <div class="info-row">
        ...
        <div class="info-col" style="text-align: right;">
            ...
            <div class="info-value">
                ...
                <strong>Parent/Sponsor:</strong> {{ $sponsor->name ?? 'N/A' }}
            </div>
        </div>
    </div>
    ...
    <div class="summary-card">
        <div class="summary-row">
            ...
            <div class="summary-value">P {{ number_format((float)$balance['total_invoiced'], 2) }}</div>
        </div>
        <div class="summary-row">
            <div class="summary-label">Total Paid:</div>
            <div class="summary-value">P {{ number_format((float)$balance['total_paid'], 2) }}</div>
        </div>
        <div class="summary-row balance">
            <div class="summary-label">Current Balance:</div>
            <div class="summary-value {{ (float)$balance['balance'] > 0 ? 'balance-positive' : 'balance-zero' }}">
                P {{ number_format((float)$balance['balance'], 2) }}
            </div>
        </div>
    </div>
            ...
            <tbody>
                @foreach($invoices as $invoice)
                    <tr>
                        ...
                        <td class="amount">P {{ number_format((float)$invoice->total_amount, 2) }}</td>
                        <td class="amount">P {{ number_format((float)$invoice->amount_paid, 2) }}</td>
                        <td class="amount">P {{ number_format((float)$invoice->balance, 2) }}</td>
                        <td class="center">
            ...
            <tbody>
                @foreach($payments as $payment)
                    <tr>
                        ...
                        <td class="amount">P {{ number_format((float)$payment->amount, 2) }}</td>
    ...
    @php
        ...
        foreach ($invoices as $invoice) {
            $transactionsList[] = [
                ...
                'description' => 'Invoice #' . $invoice->invoice_number . ' (' . ($invoice->year ?? 'N/A') . ')',
        ...
        foreach ($transactionsList as $index => $transaction) {
            ...
            $transactionsList[$index]['balance'] = $runningBalance;
        }

        $transactions = collect($transactionsList);
    @endphp
            ...
            <tbody>
                @foreach($transactions as $transaction)
                    <tr>
                        ...
                        <td class="amount">{{ $transaction['debit'] > 0 ? 'P ' . number_format($transaction['debit'], 2) : '-' }}</td>
                        <td class="amount">{{ $transaction['credit'] > 0 ? 'P ' . number_format($transaction['credit'], 2) : '-' }}</td>
                        <td class="amount">P {{ number_format($transaction['balance'], 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
