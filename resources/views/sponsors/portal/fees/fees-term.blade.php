...
<div class="container-fluid px-0">
    @forelse($children as $child)
        @php
            ...
            $balance = (float)$child->feeBalance['balance'];
            $totalPaid = (float)$child->feeBalance['total_paid'];
            $totalInvoiced = (float)$child->feeBalance['total_invoiced'];
        @endphp

        <div class="sponsor-container mb-4">
            <!-- Balance Header -->
            <div class="balance-header">
                <div class="row align-items-center g-3">
                    <div class="col-auto">
                        <div class="child-avatar" style="width: 60px; height: 60px; font-size: 24px;">
                            @if ($child->photo_path)
                    ...
                    <div class="col">
                        <h5 class="mb-1" style="position: relative; z-index: 1;">{{ $child->full_name }}</h5>
                        <small style="opacity: 0.85; position: relative; z-index: 1;">
                            <i class="bx bx-buildings me-1"></i>
                            {{ $child->currentClass ? $child->currentClass->name : 'No Class Assigned' }}
                        </small>
                    </div>
            ...
            <div class="sponsor-body">
                ...
                @if ($child->invoice)
                    <div class="invoice-meta">
                        <div class="invoice-meta-item">
                            ...
                            <div class="invoice-meta-value">{{ $child->invoice->invoice_number }}</div>
                        </div>
                        <div class="invoice-meta-item">
                            <div class="invoice-meta-label">Issue Date</div>
                            <div class="invoice-meta-value">{{ $child->invoice->issued_at ? $child->invoice->issued_at->format('d M Y') : 'N/A' }}</div>
                        </div>
                        <div class="invoice-meta-item">
                            <div class="invoice-meta-label">Due Date</div>
                            <div class="invoice-meta-value">{{ $child->invoice->due_date ? $child->invoice->due_date->format('d M Y') : 'N/A' }}</div>
                        </div>
                        <div class="invoice-meta-item">
                            <div class="invoice-meta-label">Status</div>
                            <div class="invoice-meta-value">
                                <span class="{{ $child->invoice->status_color }}">
                                    {{ ucfirst($child->invoice->status) }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="subject-table-container">
                        <table class="subject-table">
                            ...
                            <tbody>
                                @foreach ($child->invoice->items as $item)
                                    <tr>
                                        <td>
                            ...
                            <tfoot style="background: #f8f9fc;">
                                <tr>
                                    ...
                                    <td class="text-end">P {{ number_format((float)$child->invoice->subtotal_amount, 2) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="text-end fw-semibold">Total Discount</td>
                                    <td class="text-end text-success">- P {{ number_format((float)$child->invoice->discount_amount, 2) }}</td>
                                </tr>
                                <tr style="background: #e8f4fd;">
                                    <td colspan="3" class="text-end fw-bold">Total Amount</td>
                                    <td class="text-end fw-bold">P {{ number_format((float)$child->invoice->total_amount, 2) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="text-end fw-semibold">Amount Paid</td>
                                    <td class="text-end text-success fw-semibold">P {{ number_format((float)$child->invoice->amount_paid, 2) }}</td>
                                </tr>
                                <tr style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%);">
                                    <td colspan="3" class="text-end fw-bold">Balance Due</td>
                                    <td class="text-end fw-bold">P {{ number_format((float)$child->invoice->balance, 2) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                @else
                    <div class="empty-state">
                        ...
                        <p>An invoice has not been generated for this term yet.</p>
                    </div>
                @endif
                ...
                @if ($child->payments && $child->payments->count() > 0)
                    <div class="subject-table-container">
                        <table class="subject-table">
                            <thead>
                                <tr>
                ...
                @else
                    <div class="empty-state">
                        ...
                        <p>No payments have been made for this term yet.</p>
                    </div>
                @endif

                <!-- Download Statement -->
                <div class="d-flex justify-content-end mt-4 pt-4 border-top">
                    <a href="{{ route('sponsor.statement-pdf', ['student' => $child->id]) }}" class="download-btn" target="_blank">
        ...
        <div class="sponsor-container">
            <div class="sponsor-body">
                <div class="empty-state">
                    ...
                    <p>No children are enrolled for the selected term. Fee information will appear here once enrollment is complete.</p>
                </div>
            </div>
        </div>
    @endforelse
