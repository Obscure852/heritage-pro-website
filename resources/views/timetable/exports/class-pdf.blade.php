...
<body>
    <div class="header">
        ...
        <div class="report-title">{{ $className }} - Class Timetable</div>
                    ...
                    @foreach ($daySchedule as $si => $schedItem)
                        ...
                        @if ($slot)
                            ...
                            <td @if ($colspan > 1) colspan="{{ $colspan }}" @endif
                                ...
                                <div class="slot-subject">{{ Str::limit($slot['subject_name'] ?? '?', 15) }}</div>
                                <div class="slot-detail">{{ viewGetInitials($slot['teacher_name'] ?? '') }}</div>
