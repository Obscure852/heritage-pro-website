            ...
            <div class="settings-container">
                ...
                <div class="settings-body">
                    ...
                    <ul class="nav nav-tabs-custom d-flex justify-content-start mb-3" role="tablist">
                        ...
                        @can('manage-academic')
                            @foreach (($valueAdditionMappingDataSets ?? collect()) as $mappingData)
                                @php
                                    $mappingSchoolType = strtolower($mappingData['school_type']);
                                    $valueAdditionTabId = 'value-addition-mapping-' . $mappingSchoolType;
                                @endphp
                                <li class="nav-item">
                                    <a class="nav-link" data-bs-toggle="tab" href="#{{ $valueAdditionTabId }}" role="tab">
                                        <i class="fas fa-exchange-alt me-2 text-muted"></i>
                                        {{ $mappingData['exam_type'] }} Value Addition Mapping
                                    </a>
                                </li>
                            @endforeach
                            <li class="nav-item">
                            ...
                            <div class="filter-section">
                                ...
                                <div class="filter-actions">
                                    @if (!session('is_past_term'))
                                        @if (in_array(\App\Models\SchoolSetup::normalizeType($school_data->type ?? null), [\App\Models\SchoolSetup::TYPE_PRIMARY, \App\Models\SchoolSetup::TYPE_PRE_F3, \App\Models\SchoolSetup::TYPE_K12], true))
                        ...
                        @can('manage-academic')
                            @foreach (($valueAdditionMappingDataSets ?? collect()) as $mappingData)
                                @php
                                    $mappingSchoolType = strtolower($mappingData['school_type']);
                                    $valueAdditionTabId = 'value-addition-mapping-' . $mappingSchoolType;
                                    $mappingFormId = 'valueAdditionMappingForm-' . $mappingSchoolType;
                                @endphp
                                <div class="tab-pane" id="{{ $valueAdditionTabId }}" role="tabpanel">
                                    <div class="help-text">
                                        <div class="help-title">{{ $mappingData['exam_type'] }} Value Addition Subject Mapping</div>
                                        <div class="help-content">
                                            @if ($mappingData['school_type'] === 'Senior')
                                                Map JCE exam subjects to your school's senior subjects to calculate value
                                                addition
                                                (student improvement from JCE to current senior performance).
                                            @else
                                                Map PSLE exam subjects to your school's junior subjects to calculate value
                                                addition
                                                (student improvement from PSLE to current junior performance).
                                            @endif
                                            You can map one exam subject to multiple school subjects (e.g. Science to Physics,
                                            Biology, Chemistry).
                                            Subjects left unmapped will be excluded from value addition calculations.
                                        </div>
                                    </div>

                                    <form id="{{ $mappingFormId }}" class="value-addition-mapping-form">
                                        <input type="hidden" name="school_type"
                                            value="{{ $mappingData['school_type'] }}">
                                        <input type="hidden" name="exam_type"
                                            value="{{ $mappingData['exam_type'] }}">
                                        ...
                                        <div class="table-responsive">
                                            <table class="threshold-table">
                                                <thead>
                                                    <tr>
                                                        <th style="width: 5%;">#</th>
                                                        <th style="width: 30%;">{{ $mappingData['exam_type'] }}
                                                            Subject</th>
                                                        <th style="width: 25%;">Column Key</th>
                                                        <th style="width: 40%;">Maps To (School Subject)</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($mappingData['source_columns'] as $index => $column)
                                                        <tr>
                                                            <td>{{ $index + 1 }}</td>
                                                            <td>
                                                                <span class="scope-label">{{ $column['source_label'] }}</span>
                                                            </td>
                                                            <td>
                                                                <code
                                                                    style="font-size: 12px; color: #6b7280;">{{ $column['source_key'] }}</code>
                                                            </td>
                                                            <td>
                                                                <select class="form-select va-mapping-select"
                                                                    name="mappings[{{ $column['source_key'] }}][]"
                                                                    data-source-key="{{ $column['source_key'] }}"
                                                                    multiple="multiple" data-placeholder="Select subjects...">
                                                                    @foreach ($mappingData['subjects'] as $subject)
                                                                        <option value="{{ $subject->id }}"
                                                                            {{ in_array($subject->id, $mappingData['existing_mappings'][$column['source_key']] ?? []) ? 'selected' : '' }}>
                                                                            {{ $subject->name }}
                                                                            @if ($subject->abbrev)
                                                                                ({{ $subject->abbrev }})
                                                                            @endif
                                        ...
                                        <div class="form-actions">
                                            <button type="submit" class="btn btn-primary btn-loading save-va-mapping-btn">
                                                <span class="btn-text"><i class="fas fa-save"></i> Save Mappings</span>
...
@section('script')
    <script>
        ...
        // Value Addition Mapping
        @can('manage-academic')
            @if (isset($valueAdditionMappingDataSets) && ($valueAdditionMappingDataSets->count() ?? 0) > 0)
                $(document).ready(function() {
                    // Initialize Select2 on value addition mapping selects
                    $('.va-mapping-select').select2({
                        placeholder: 'Select subjects...',
                        allowClear: true,
                        width: '100%',
                    });

                    $('.value-addition-mapping-form').on('submit', function(e) {
                        e.preventDefault();

                        const form = $(this);
                        const saveBtn = form.find('.save-va-mapping-btn').get(0);
                        saveBtn.classList.add('loading');
                        saveBtn.disabled = true;

                        const mappings = {};
                        form.find('.va-mapping-select').each(function() {
                            const sourceKey = $(this).data('source-key');
                            const selectedValues = $(this).val();
                            mappings[sourceKey] = selectedValues && selectedValues.length > 0 ?
                                selectedValues : [];
                        });

                        const data = {
                            school_type: form.find('input[name="school_type"]').val(),
                            exam_type: form.find('input[name="exam_type"]').val(),
                            mappings: mappings,
                        };

                        $.ajax({
                            url: "{{ route('assessment.value-addition-mappings.store') }}",
                            type: 'POST',
                            ...
                            error: function(xhr) {
                                ...
                                let message = 'An error occurred while saving mappings.';
