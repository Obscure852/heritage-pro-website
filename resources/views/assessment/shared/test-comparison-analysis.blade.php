...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('title')
            {{ $klass->name }} Test Comparison Analysis
        @endslot
    @endcomponent
    ...
    <style>
        ...
        @media print {
            html, body { margin: 0; padding: 0; width:100%; height:100%; font-size:10px; line-height:normal; overflow:visible; }
            ...
            .table-responsive { width:100% !important; margin:0; padding:0; page-break-inside:avoid; }
            .table, .table th, .table td { border-collapse: collapse; overflow:visible; word-wrap:break-word; }
                ...
                <div class="card-body">
                    <h5 class="mb-2">
                        ...
                        @else
                            End Of {{ $test->name ?? '' }}
                        @endif
                        Term {{ $test->term->term ?? '' }}, {{ $test->term->year ?? '' }} Comparison Analysis
                    </h5>
                            ...
                            <tbody>
                                @foreach(['A','B','C','D','E','U'] as $grade)
                                    <tr>
                                        <td>{{ $grade }}</td>
                                        @foreach($subjects as $subject)
                                            <td class="text-center">{{ $gradeCounts[$subject]['prev'][$grade] ?? 0 }}</td>
                                            <td class="text-center">{{ $gradeCounts[$subject]['curr'][$grade] ?? 0 }}</td>
                                        @endforeach
                                    </tr>
                                @endforeach
                                <tr>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    @php
                                        ...
                                        $prevAB    = (($prevGradeCounts['A'] ?? 0) + ($prevGradeCounts['B'] ?? 0)) / max($totalPrev,1) * 100;
                                        $prevABC   = (($prevGradeCounts['A'] ?? 0) + ($prevGradeCounts['B'] ?? 0) + ($prevGradeCounts['C'] ?? 0)) / max($totalPrev,1) * 100;
                                        $prevDEU   = (($prevGradeCounts['D'] ?? 0) + ($prevGradeCounts['E'] ?? 0) + ($prevGradeCounts['U'] ?? 0)) / max($totalPrev,1) * 100;
                                    @endphp
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    @php
                                        ...
                                        $currAB    = (($currGradeCounts['A'] ?? 0) + ($currGradeCounts['B'] ?? 0)) / max($totalCurr,1) * 100;
                                        $currABC   = (($currGradeCounts['A'] ?? 0) + ($currGradeCounts['B'] ?? 0) + ($currGradeCounts['C'] ?? 0)) / max($totalCurr,1) * 100;
                                        $currDEU   = (($currGradeCounts['D'] ?? 0) + ($currGradeCounts['E'] ?? 0) + ($currGradeCounts['U'] ?? 0)) / max($totalCurr,1) * 100;
                                    @endphp
