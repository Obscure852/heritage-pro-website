        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="report-card">
                    <div class="row">
                        <h5>{{ $klass->grade->name }} End of Term Overall Grade Analysis - Term
                            {{ $currentTerm->term }},
                            {{ $currentTerm->year }}</h5>
                        <div class="col-md-12 col-lg-12">
                                            ...
                                            <tbody>
                                                ...
                                                @foreach ($subjectGradeCounts as $subject => $counts)
                                                    <tr>
                                                        ...
                                                        {{-- Percentage metrics with T = M%+F% (or recomputed) --}}
                                                        @foreach (['AB%', 'ABC%', 'ABCD%', 'DEU%', 'X%'] as $p)
                                                            <td>{{ $counts[$p]['M'] }}%</td>
                                                            <td>{{ $counts[$p]['F'] }}%</td>
                                                            <td>
                                                                @php
                                                                    $m = $counts[$p]['M'];
                                                                    $f = $counts[$p]['F'];
                                                                    // If you want to average rather than sum, replace '+' with '/2' or your logic:
                                                                    $t = $m + $f;
                                                                @endphp
                                                                {{ $t }}%
                                                            </td>
...
<script>
    ...
    var gradeDistributionChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: Object.keys(subjectGradeCounts), // Subject names
            datasets: [{
                    label: 'Male - A Grades',
