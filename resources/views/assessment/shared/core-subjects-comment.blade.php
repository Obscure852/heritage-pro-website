            ...
            <div class="settings-container">
                <div class="settings-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h3><i class="fas fa-comment-alt me-2"></i>Subject Remarks</h3>
                            <p>Add remarks for {{ $student->fullName ?? 'student' }}</p>
                        </div>
                        @if (isset($subjectExam->pivot) && ($subjectExam->pivot->score || $subjectExam->pivot->grade))
                            ...
                            <div class="d-flex">
                                ...
                                @endif
                                <span class="score-badge">{{ $studentNumber }} / {{ $student->currentClass()->students->count() ?? '' }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                <div class="settings-body">
                    <form class="needs-validation" method="post" action="{{ route('assessment.new-remark') }}" novalidate>
                        @csrf
                        <input type="hidden" name="student_id" value="{{ $student->id }}">
                        <input type="hidden" name="grade_subject_id" value="{{ $klass->subject->id }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="remarks" id="subject_teacher" rows="5" class="form-control mt-2">{{ old('remarks',\App\Models\SubjectComment::where('student_id', $student->id)->where('grade_subject_id', $klass->subject->id)->where('term_id', session('selected_term_id', $klass->term->id))->first()->remarks ?? '') }}</textarea>
                        </div>
                        ...
                        <div class="form-actions">
                            ...
                            @if ($index < count(explode(',', $studentIds)) - 1)
                                <button type="submit" name="action" value="save_and_next" class="btn-next">
            ...
            if (form) {
                ...
                form.addEventListener('submit', function(e) {
                    if (clickedButton) {
                        ...
                        // Also remove d-none from spinner manually
                        const spinner = clickedButton.querySelector('.btn-spinner');
                        if (spinner) {
