    ...
    @if (!empty($klass))
        <div style="background: #4e73df; border-radius: 3px; color: white; padding: 0.5rem; margin: 2px;"
            ...
            <div class="col-md-6 d-flex justify-content-end">
                ...
                <button type="button" class="btn btn-sm btn-link text-info text-decoration-none" data-bs-toggle="modal"
                    data-bs-target="#recalculateModal" id="recalculateBtn" title="Recalculate Marks for This Grade">
        ...
        <form method="POST" action="{{ route('assessment.update-marks') }}">
            ...
            <div class="table-scroll-wrapper">
                ...
                <div class="table-responsive">
                    <table id="markbook-class" class="table align-middle" style="width: max-content;">
                        @php
                                ...
                                <tr
                                    ...
                                    <td>
                                        ...
                                        <a class="comment-link"
                                            href="{{ route('assessment.core-subject-remarks', ['studentId' => $student->id, 'id' => $klass->id, 'studentIds' => implode(',', $studentIds), 'index' => $rowIndex]) }}">
                                    ...
                                    @if ($caTests->isNotEmpty())
                                        @foreach ($caTests as $caIndex => $test)
                                            ...
                                            @endphp
                                            <td class="ca-cell {{ $loop->first ? 'ca-cell-first' : '' }}">
                                                <input type="hidden"
                                    ...
                                    @if ($examTests->isNotEmpty())
                                        @foreach ($examTests as $examIndex => $test)
                                            ...
                                            @endphp
                                            <td class="exam-cell {{ $loop->first ? 'exam-cell-first' : '' }}">
                                                <input type="hidden"
                                                    name="students[{{ $student->id }}][tests][{{ $test->id }}][out_of]"
                                                    value="{{ $test->out_of }}">
                                                <input type="text" class="form-control form-control-sm score-input"
                ...
                <div class="scroll-progress bottom">
                    <div class="scroll-progress-bar"></div>
                </div>
            </div>
            @if (Auth::user()->can('manage-academic') || !session('is_past_term'))
...
<script>
    $(document).ready(function() {
        if (window._typewriterTimeouts) {
            window._typewriterTimeouts.forEach(clearTimeout);
        }
        ...
        if (classInfoElement) {
            ...
            function addBlinkingCursor() {
                ...
                var cursorTid = setTimeout(function() {
                    cursor.remove();
                }, 3000);
                window._typewriterTimeouts.push(cursorTid);
            }
        ...
        // Score input validation - use event delegation with namespaced events
        ...
        // Sortable headers with visual feedback - use event delegation
        $(document).off('click.sortableHeader', '.sortable-header')
            .on('click.sortableHeader', '.sortable-header', function() {
        ...
        // Clear any existing modal setup timeout
        ...
        window._modalSetupTimeout = setTimeout(function() {
            ...
            $('#recalculateModal').off('show.bs.modal.markbook').on('show.bs.modal.markbook',
                function() {
            ...
            $('#recalculateModal').off('hidden.bs.modal.markbook').on('hidden.bs.modal.markbook',
                function() {
