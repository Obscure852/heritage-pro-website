...
<body>
    @foreach ($reportCards as $reportCard)
        ...
        <div style="margin-top:20px;" class="container page-cover">
            <div class="row">
                @php
                    ...
                    if (file_exists($logoFullPath)) {
                        ...
                        $logoBase64 =
                            'data:image/' . $extension . ';base64,' . base64_encode(file_get_contents($logoFullPath));
                    }
                @endphp
            ...
            <div class="row mt-4">
                <div class="col-12 text-center">
                    <p><strong>Name:</strong> {{ $reportCard['student']->fullName ?? '' }}</p>
                    ...
                    <p><strong>Class:</strong> {{ $reportCard['currentClass']->name }}</p>
                                    ...
                                    @foreach ($examTests as $test)
                                        @foreach ($gradeOptions as $option)
                                            <td class="criteria-option">
                                                ...
                                                @endphp
                                                @if ($assessment)
                                                    <img src="{{ public_path('storage/branding/check.jpeg') }}"
                                                        width="12px" height="12px" alt="tick">
                                                @else
                                                    <span>&nbsp;</span>
                                                @endif
                                            </td>
            ...
            <table class="table table-bordered table-sm">
                <tr>
                    ...
                    <th style="text-align: left; width: 50%;">
                        Teacher's Name: {{ $reportCard['currentClass']->teacher->fullName ?? 'N/A' }}</th>
                </tr>
                <tr>
                    <td colspan="2">
            ...
            <table class="remarks" style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="width: 33%; text-align: left; vertical-align: top;">
                        ...
                        @if (!empty($reportCard['currentClass']->teacher->signature_path))
                            <img src="{{ public_path($reportCard['currentClass']->teacher->signature_path) }}"
                                alt="{{ $reportCard['currentClass']->teacher->fullName }}'s signature"
                                style="height: 60px;">
                        @else
