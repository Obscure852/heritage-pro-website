...
<head>
    ...
    <style>
        ...
        .header { background: linear-gradient(135deg, #0f766e 0%, #0f172a 100%); color: #fff; padding: 22px 24px; border-radius: 6px; }
...
<body>
    <div class="page">
        <div class="header">
            <h1>Heritage CRM Invoice</h1>
            <p>{{ $document->invoice_number }} · {{ $accountName }} · {{ $document->invoice_date?->format('d M Y') }}</p>
        </div>
        ...
        <table class="grid">
            <tr>
                <td>
                    <div class="card">
                        <div class="kicker">Invoice Summary</div>
                        <div class="meta-row"><strong>Invoice number:</strong> {{ $document->invoice_number }}</div>
                        <div class="meta-row"><strong>Status:</strong> {{ ucfirst($document->status) }}</div>
                        <div class="meta-row"><strong>Invoice date:</strong> {{ $document->invoice_date?->format('d M Y') ?: 'Not set' }}</div>
