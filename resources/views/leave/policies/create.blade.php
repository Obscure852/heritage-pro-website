...
@section('title')
    Add Policy Year - {{ $leaveType->name }}
@endsection
@section('css')
    <style>
        .form-container {
...
@section('content')
    ...
    <div class="form-container">
        <div class="page-header">
            <h1 class="page-title">Add Policy Year</h1>
        ...
        <div class="help-text">
            <div class="help-title">Configure Yearly Policy</div>
            <div class="help-content">
                Set up how leave balances for <strong>{{ $leaveType->name }}</strong> are managed for a specific year.
                Configure balance allocation mode, carry-over rules, and proration settings.
                Leave year starts in <strong>{{ DateTime::createFromFormat('!m', $leaveYearStartMonth)->format('F') }}</strong>.
            </div>
        </div>

        <form class="needs-validation" method="post" action="{{ route('leave.policies.store', $leaveType) }}" novalidate>
            @csrf

            <h3 class="section-title">Year & Balance Settings</h3>
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label" for="leave_year">Leave Year <span class="text-danger">*</span></label>
                    <select class="form-select @error('leave_year') is-invalid @enderror" name="leave_year" id="leave_year" required>
                        <option value="">Select Year</option>
                        @foreach($years as $year)
                            @if(!in_array($year, $existingYears))
                                <option value="{{ $year }}" {{ old('leave_year', $currentYear) == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endif
                        @endforeach
                    </select>
                    <div class="form-text">Select a year that doesn't already have a policy configured</div>
                    @error('leave_year')
                ...
                <div class="form-group">
                    ...
                    <select class="form-select @error('balance_mode') is-invalid @enderror" name="balance_mode" id="balance_mode" required>
                        <option value="allocation" {{ old('balance_mode', 'allocation') == 'allocation' ? 'selected' : '' }}>Allocation (Full balance at year start)</option>
                        <option value="accrual" {{ old('balance_mode') == 'accrual' ? 'selected' : '' }}>Accrual (Balance accumulates monthly)</option>
                    </select>
                    @error('balance_mode')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-grid" style="margin-top: 16px;">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select @error('prorate_new_employees') is-invalid @enderror" name="prorate_new_employees" id="prorate_new_employees" required>
                        <option value="1" {{ old('prorate_new_employees', '1') == '1' ? 'selected' : '' }}>Yes (Entitlement based on employment start date)</option>
                        <option value="0" {{ old('prorate_new_employees') == '0' ? 'selected' : '' }}>No (Full entitlement regardless of join date)</option>
                    </select>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select class="form-select @error('carry_over_mode') is-invalid @enderror" name="carry_over_mode" id="carry_over_mode" required>
                        <option value="none" {{ old('carry_over_mode', 'none') == 'none' ? 'selected' : '' }}>None (No carry-over)</option>
                        <option value="limited" {{ old('carry_over_mode') == 'limited' ? 'selected' : '' }}>Limited (Up to specified days)</option>
                        <option value="full" {{ old('carry_over_mode') == 'full' ? 'selected' : '' }}>Full (All unused days carry over)</option>
                    </select>
            ...
            <div class="form-grid" style="margin-top: 16px;">
                <div class="form-group" id="carry_over_expiry_group" style="display: none;">
                    ...
                    <input type="number"
                        ...
                        placeholder="e.g., 3"
                        value="{{ old('carry_over_expiry_months') }}"
                        min="1" max="12">
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn btn-primary btn-loading">
                    <span class="btn-text"><i class="fas fa-save"></i> Save Policy</span>
