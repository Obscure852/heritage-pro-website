...
@section('title')
    Optional Class Students | Academic Management
@endsection
...
@section('content')
    ...
    <div class="settings-container">
        <div class="settings-header">
            ...
            <p>View and manage students allocated to this optional class</p>
            <div class="class-info">
                ...
                <span class="info-badge">
                    <i class="fas fa-user-graduate"></i> <span id="studentCount">{{ $class->students->count() ?? 0 }}</span> Students
                </span>
            </div>
        </div>
        ...
        <div class="settings-body">
            @if (session('is_past_term'))
                <div class="help-text warning">
                    <div class="help-title"><i class="bx bx-info-circle me-1"></i>Past Term View</div>
                    <div class="help-content">
                        You are viewing data from a previous term. Authorized teachers and supervisors can still move or remove students here, so changes should be made carefully.
                    </div>
                </div>
            @else
                <div class="help-text">
                    ...
                    <div class="help-content">
                        Select students using the checkboxes to perform bulk actions like moving to another class or removing from this class.
                ...
                <div class="col-lg-4 col-md-12 text-lg-end text-md-start mt-lg-0 mt-3">
                    <div class="bulk-actions justify-content-lg-end">
                        @can('optional-teacher', $class)
                            ...
                            <button type="button" class="btn-bulk-action remove" id="deleteSelectedBtn" style="display: none;">
            ...
            <div class="table-responsive">
                <form id="deleteStudentsForm" method="POST" action="{{ route('optional.delete-multiple-students', $class->id) }}">
                    @csrf
                    @method('DELETE')
                            ...
                            @if (!empty($class))
                                @foreach ($class->students as $index => $student)
                                    <tr class="student-row"
                                        ...
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input student-checkbox" name="students[]"
                                                    value="{{ $student->id }}"
                                                    data-grade-id="{{ optional($student->currentClass())->grade_id }}">
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>{{ $class->name }}</td>
                                        <td>{{ optional($student->currentClass())->name ?? 'N/A' }}</td>
                                        <td>{{ $student->currentClass()?->grade?->name ?? 'N/A' }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
            ...
            <div class="pagination-container">
                <div class="text-muted" id="results-info">
                    Showing <span id="showing-from">0</span> to <span id="showing-to">0</span> of <span id="total-count">{{ $class->students->count() ?? 0 }}</span> students
                </div>
        ...
        <div class="modal fade" id="moveStudentsModal" tabindex="-1" aria-labelledby="moveStudentsModalLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form id="moveStudentsForm" method="POST" action="{{ route('optional.move-multiple-students', $class->id) }}">
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="moveStudentsModalLabel"><i class="bx bx-transfer me-2"></i>Move Selected Students</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="new_class_id" class="form-label">Select the new optional class for the selected students</label>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary" onclick="return confirm('Are you sure you want to move the selected student(s)?');">
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            let baseClassesByGradeRoute = "{{ route('optional.optional-classes-by-grade', ['gradeId' => ':gradeId']) }}";
            ...
            $('#deleteSelectedBtn').on('click', function() {
                ...
                @if (session('is_past_term'))
                    var confirmMessage = 'Are you sure you want to remove the selected students from this past term? All their marks and comments for this term will be lost.';
                @else
                    var confirmMessage = 'Are you sure you want to remove the selected students? All their marks and comments will be lost.';
                @endif
            ...
            @can('optional-teacher', $class)
                $('#moveSelectedBtn').on('click', function() {
                    ...
                    let finalUrl = baseClassesByGradeRoute.replace(':gradeId', uniqueGradeId);
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                $('#new_class_id').empty();
                                $('#new_class_id').append('<option value="">-- Select Class --</option>');
                                ...
                                let moveModal = new bootstrap.Modal(document.getElementById('moveStudentsModal'), {
                                    keyboard: false
                                });
                                moveModal.show();
                            } else {
                                alert('Error fetching classes: ' + (response.message || 'Unknown error.'));
                            }
                        },
