...
@section('title')
    {{ $conversation->student->full_name ?? 'Conversation' }}
@endsection
...
@section('content')
    @component('components.breadcrumb')
        @slot('li_1')
            <a href="{{ route('lms.messaging.inbox') }}">Messages</a>
        @endslot
        @slot('title')
            Conversation
        @endslot
    ...
    <div class="chat-wrapper">
        <div class="chat-header">
            <div class="chat-header-left">
                <a href="{{ route('lms.messaging.inbox') }}" class="back-btn">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <div class="chat-user-avatar">
                    {{ strtoupper(substr($conversation->student->first_name ?? 'S', 0, 1)) }}
                </div>
                <div class="chat-user-info">
                    <h5>{{ $conversation->student->full_name ?? 'Unknown Student' }}</h5>
                    <small>{{ $conversation->course->title ?? $conversation->subject ?? 'General' }}</small>
                </div>
            </div>
            <div class="chat-header-actions">
                <form action="{{ route('lms.messaging.archive', $conversation) }}" method="POST" class="d-inline">
                    @csrf
        ...
        <div class="chat-messages" id="chatMessages">
            ...
            @foreach ($messages as $message)
                @php
                    ...
                    $isSent = $message->isSentByInstructor();
                @endphp
                ...
                <div class="message-row {{ $isSent ? 'sent' : '' }}">
                    <div class="message-avatar">
                        @if ($isSent)
                            {{ strtoupper(substr($instructor->firstname ?? 'T', 0, 1)) }}
                        @else
                            {{ strtoupper(substr($conversation->student->first_name ?? 'S', 0, 1)) }}
                        @endif
                    </div>
        ...
        <div class="chat-input-area">
            <form action="{{ route('lms.messaging.reply', $conversation) }}" method="POST"
