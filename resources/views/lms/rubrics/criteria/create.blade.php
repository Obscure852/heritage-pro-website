...
@section('content')
    ...
    <form action="{{ route('lms.rubrics.criteria.store', $rubric) }}" method="POST" id="criterionForm">
        @csrf
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="form-container">
                    <div class="page-header">
                        <h1 class="page-title">Add Criterion</h1>
                    </div>

                    <div class="help-text">
                        <div class="help-title">Define a Grading Criterion</div>
                        <div class="help-content">
                            Create a criterion with multiple performance levels. Each level represents
                            a different quality of work and its corresponding points.
                        </div>
                    </div>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                                value="{{ old('title') }}" placeholder="e.g., Content Quality" required>
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="max_points" id="maxPoints"
                                ...
                                value="{{ old('max_points', 10) }}" min="1" max="1000" required>
                            @error('max_points')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control" rows="2" placeholder="What does this criterion evaluate?">{{ old('description') }}</textarea>
                    </div>
            ...
            <div class="col-lg-4">
                <div class="rubric-info">
                    <h4>Adding to Rubric</h4>
                    <p class="rubric-title">{{ $rubric->title }}</p>
                </div>

                <div class="summary-card">
                    <h3><i class="fas fa-info-circle me-2"></i>Current Rubric</h3>
                    <div class="summary-stat">
                        <span class="label">Existing Criteria</span>
                        <span class="value">{{ $rubric->criteria()->count() }}</span>
                    </div>
                    <div class="summary-stat">
                        <span class="label">Current Total Points</span>
                        <span class="value">{{ number_format($rubric->total_points) }}</span>
                    </div>
                </div>
                ...
                <div class="summary-card">
                    ...
                    <ul class="text-muted small mb-0" style="padding-left: 20px;">
                        <li class="mb-2">Use descriptive level titles (Excellent, Good, Fair, Poor)</li>
                        <li class="mb-2">Each criterion needs at least 2 levels</li>
                        <li class="mb-2">Points should decrease as quality decreases</li>
...
@section('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Add default levels
            addLevel('Excellent', '', 10);
            addLevel('Good', '', 7);
            addLevel('Satisfactory', '', 5);
            addLevel('Needs Improvement', '', 2);
        });

        function addLevel(title = '', description = '', points = '') {
